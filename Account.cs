...
namespace Bank_MNGMT
{
    public partial class Account : Form
    {
        public Account()
        {
            ...
            // Event baglamak
            txtCustName.TextChanged += textBox1_TextChanged;
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            using (SqlConnection con = new SqlConnection("Data Source=BLACKGHOST-PC;Initial Catalog=\"bank db\";Integrated Security=True;Encrypt=False"))
            {
                ...
                if (string.IsNullOrWhiteSpace(cmbAcc.Text) || string.IsNullOrWhiteSpace(txtBal.Text) ||  string.IsNullOrWhiteSpace(txtCustName.Text)|| string.IsNullOrWhiteSpace(dateTimePicker2.Text))
                {
                ...
                {
                    int account_id = (int)btnAdd.Tag;
                    SqlCommand cmd = new SqlCommand("UPDATE accounts SET actype=@actype, balance=@balance, date_opened=@date_opened, customer_name=@customer_name WHERE account_id=@account_id", con);
                    cmd.Parameters.AddWithValue("@account_id", account_id);
                    cmd.Parameters.AddWithValue("@actype", cmbAcc.SelectedItem.ToString());
                    cmd.Parameters.AddWithValue("@balance", txtBal.Text);
                    cmd.Parameters.AddWithValue("@date_opened", dateTimePicker2.Value);
                ...
                {
                    SqlCommand cmd = new SqlCommand("INSERT INTO accounts (actype, balance, date_opened, customer_name) VALUES (@actype, @balance, @date_opened, @customer_name)", con);
                    cmd.Parameters.AddWithValue("@actype", cmbAcc.SelectedItem.ToString());
                    cmd.Parameters.AddWithValue("@balance", txtBal.Text);
                    cmd.Parameters.AddWithValue("@date_opened", dateTimePicker2.Value);
                    cmd.Parameters.AddWithValue("@customer_name", txtCustName.Text);
        ...
        {
            // TODO: This line of code loads data into the 'bank_dbDataSet9.accounts' table. You can move, or remove it, as needed.
            this.accountsTableAdapter1.Fill(this.bank_dbDataSet9.accounts);
            // TODO: This line of code loads data into the 'bank_dbDataSet1.accounts' table. You can move, or remove it, as needed.
            this.accountsTableAdapter.Fill(this.bank_dbDataSet1.accounts);
            RefreshData();
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (cellValue == null || !int.TryParse(cellValue.ToString(), out int Account_id))
                return;
            ...
            else if (columnName == "Edit")
            {
                ...
                string Account_Type = dataGridView1.Rows[e.RowIndex].Cells[2].Value.ToString();
                string balance = dataGridView1.Rows[e.RowIndex].Cells[3].Value.ToString();
                string date_opened = dataGridView1.Rows[e.RowIndex].Cells[4].Value.ToString();

                cmbAcc.Text = Account_Type;
                txtBal.Text = balance;
                txtCustName.Text = customer_name;

                btnAdd.Text = "Update";
                btnAdd.Tag = Account_id;
            }
        } 
